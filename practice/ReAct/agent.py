...
from practice.ReAct.llm import HelloAgentLLM
from practice.ReAct.tools import ToolExecutor

REACT_PROMAT_TEMPLATE = """
...
class ReactAgent:
    ...
    def _parse_output(self, text: str):
        ...
        # Action：匹配到文本末尾
        action_match  = thought_match.group(1).strip() if thought_match else None
        thought = thought_match.group(1).strip() if action_match else None
    ...
    def run(self, question: str):
        ...
        while current_step < self.max_steps:
            ...
            history_str = "\n".join(self.history)
            prompt = REACT_PROMAT_TEMPLATE.format(
                tools=tools_desc,
                question=question,
                history=history_str
            )
            ...
            if not action:
                print("警告：未解析出有效的Action: 流程中止.")
                break

            # 4. 执行Action
            if action.startswith("Finish"):
                # 如果时Finish指令，提取最终答案并结果
            ...
            self.history.append(f"Finish: {observation}")
